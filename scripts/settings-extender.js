...
(() => {
    ...
    function createExtraInputTypes() {
        ...
        FilePickerImage._init = ($html) => {
            ...
            $filePickers.each((idx, input) => {
                ...
                const $filePickerButton = $(
                    `<button type=button class=file-picker title="Browse Files" tabindex=-1>` + `<i class="fas fa-file-import fa-fw"></i></button>`
                );
        ...
        DirectoryPicker._init = ($html) => {
            ...
            $directoryPickers.each((idx, input) => {
                const $input = $(input);
                const $browseButton = $(
                    `<button type=button title="Browse Directories" tabindex=-1>` + `<i class="fas fa-file-import fa-fw"></i></button>`
                );
    ...
    function isNewestVersionEnabled() {
        ...
        return oldVersion.major < curVersion.major || oldVersion.minor < curVersion.minor || oldVersion.patch < curVersion.patch;
    }
